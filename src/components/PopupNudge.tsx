...
export default function PopupNudge({ nudge, severity = "low", onClose }: { nudge: string; severity?: string; onClose: () => void; }) {
    ...
    <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 9999 }}>
      <div style={{ width: 340, background: "#fff", borderRadius: 12, border: "1px solid #e7e5e4", boxShadow: "0 6px 24px rgba(0,0,0,0.12)", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, padding: 10 }}>
          ...
          <button onClick={() => { window.location.href = '#'; onClose(); }} style={{ background: "#292524", color: "#fff", border: "none", padding: "6px 10px", borderRadius: 999 }}>Start 20m</button>
        </div>
      </div>
    </div>
  );
